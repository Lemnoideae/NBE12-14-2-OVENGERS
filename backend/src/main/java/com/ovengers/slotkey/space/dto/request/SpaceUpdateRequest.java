...
public record SpaceUpdateRequest(
        ...
        @Schema(
                description = "공간 사진 수정 시 직접 입력하지 않고 비워두어야 합니다. 비어 있지 않은 값을 전송하면 VALIDATION_FAILED(400) 오류가 발생합니다. 사진 교체는 별도 사진 업로드 API(PUT /api/v1/admin/spaces/{spaceId}/image)로 수행합니다.",
                example = ""
        )
